...
const NavBar = () => {
  ...
  return (
    <div className=''>
      <div className={`flex flex-col md:flex-row lg:flex-row justify-between items-center py-5 px-3 md:px-6 transition-all ease-in-out duration-500 `}>
      ...
      <div className={`absolute left-0 right-0 bg-opacity-80 p-5 flex flex-col items-center transition-all duration-300 ease-in-out ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        {buttons.map((button) => {
          if (button.primary) {
            return (
